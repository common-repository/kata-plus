	...
	class Kata_Plus_Fast_Mode extends Kata_Plus_Admin_Panel {
			...
			if ( parent::current_screen()['base_name'] == 'fast mode' && self::$step ) {
				...
				wp_localize_script(
					...
					array(
						'ajax' => array(
							...
							'notice' => array(
								'importing' => __( 'Importing content, Please Wait!', 'kata-plus-pro' ),
								'error'     => __( 'Import progress failed.', 'kata-plus-pro' ),
								'success'   => __( 'Import progress completed.', 'kata-plus-pro' ),
							),
						),
					)
				);
				...
				if ( '5' === self::$step ) {
					...
					wp_localize_script(
						...
						array(
							'translation' => array(
								'activate'           => esc_html__( 'Activate', 'kata-plus-pro' ),
								'deactivate'         => esc_html__( 'Deactivate', 'kata-plus-pro' ),
								'fail-plugin-action' => esc_html__( 'There was a problem with your action. Please try again or reload the page.', 'kata-plus-pro' ),
							),
						)
					);
				}
		...
		public static function help_modual() {
			...
			<div class="kt-fst-hlp-overlay" style="display:none;">
				<div class="kt-fst-hlp-wrapper">
					...
					<h3 class="kt-fst-hlp-title"><?php echo esc_html__( 'help', 'kata-plus-pro' ); ?></h3>
					<div class="kt-fst-hlp-inner-alert"><?php echo esc_html__( 'The Fast Mode is a mode by which you can create a website as fast as possible. It does not have anything to do with the website load speed or performance.', 'kata-plus-pro' ); ?></div>
					<div class="kt-fst-hlp-inner-content">
						<?php
						switch ( $steps ) {
							case '1':
								?>
								<h5 class="content-title"><?php echo esc_html__( 'Launch the website right now.', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'Choosing the type of website. In this step, you have to specify the type of website you want to run. Please make this choice carefully because it affects the next steps and some of them will be dependent on your choice on this step. It may cause some changes, adding, or removals in some of the options of the following steps.', 'kata-plus-pro' ); ?></p>
								<?php
								break;
							case '2':
								?>
								<h5 class="content-title"><?php echo esc_html__( 'Site title', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'The Site Title appears across the top of every page of a site', 'kata-plus-pro' ); ?></p>
								<h5 class="content-title"><?php echo esc_html__( 'Site tagline', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'The title is typically the name of your site', 'kata-plus-pro' ); ?></p>
								<h5 class="content-title"><?php echo esc_html__( 'Admin Email', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'Your website\'s email address is used by WordPress to send important email notifications. For example, when a new user account is created, an auto update is installed, and for comment moderation notices. The admin user\'s email address is used to recover lost password and notifications about their account', 'kata-plus-pro' ); ?></p>
								<?php
								break;
							case '3':
								?>
								<h5 class="content-title"><?php echo esc_html__( 'Site Icon', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'A favicon (universal term) or Site Icon (WordPress terminology) is short for favorites icon. It is an icon associated with a website to be displayed with bookmarks, in the URL bar, on tabs, and anywhere else where a website has to be identified visually.', 'kata-plus-pro' ); ?></p>
								<h5 class="content-title"><?php echo esc_html__( 'Logo', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'By choosing the website\'s logo, it will be shown wherever logo widget has been used.', 'kata-plus-pro' ); ?></p>
								<?php
								break;
							case '4':
								?>
								<h5 class="content-title"><?php echo esc_html__( 'Logo, Address and socials', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'By filling out the above form, your data regarding the phone, address, and socials will be saved in the widget and will be shown in your website. And you won\'t need to edit in widgets through your website.', 'kata-plus-pro' ); ?></p>
								<?php
								break;
							case '5':
								?>
								<h5 class="content-title"><?php echo esc_html__( 'Demo Importer', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'The Demo Importer is a tool used to import Kata\'s pre-made demos. Using this tool, you can activate the plugins needed for every demo in a few simple clicks and import all the demo data to any WordPress website.', 'kata-plus-pro' ); ?></p>
								<?php
								break;
							case '6':
								?>
								<h5 class="content-title"><?php echo esc_html__( 'Launch the website right now', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'By choosing Launch in the website, all settings will be saved and you will be redirected to the home page. ', 'kata-plus-pro' ); ?></p>
								<h5 class="content-title"><?php echo esc_html__( 'Edit website\'s content and design.', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'By choosing Edit website\'s content, you move to the next step, enabling you with further options to start the customization.', 'kata-plus-pro' ); ?></p>
								<?php
								break;
							case '7':
								?>
								<h5 class="content-title"><?php echo esc_html__( 'Homepage', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'Go to homepage customization area.', 'kata-plus-pro' ); ?></p>
								<h5 class="content-title"><?php echo esc_html__( 'Menu', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'Go to menu navigation customization area.', 'kata-plus-pro' ); ?></p>
								<h5 class="content-title"><?php echo esc_html__( 'Blog', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'Go to Blog Builder customization area.', 'kata-plus-pro' ); ?></p>
								<h5 class="content-title"><?php echo esc_html__( 'Pages', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'Show list of pages to start customize them using elementor page builder.', 'kata-plus-pro' ); ?></p>
								<h5 class="content-title"><?php echo esc_html__( 'Posts', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'Show list of posts to start customize them.', 'kata-plus-pro' ); ?></p>
								<h5 class="content-title"><?php echo esc_html__( 'Header', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'Go to Header Builder customization area.', 'kata-plus-pro' ); ?></p>
								<h5 class="content-title"><?php echo esc_html__( 'Footer', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'Go to Footer Builder customization area.', 'kata-plus-pro' ); ?></p>
								<h5 class="content-title"><?php echo esc_html__( 'Typography', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'Import Preset typography.', 'kata-plus-pro' ); ?></p>
								<h5 class="content-title"><?php echo esc_html__( 'Plugins', 'kata-plus-pro' ); ?></h5>
								<p class="content-text"><?php echo esc_html__( 'Install & activate the recommended plugins.', 'kata-plus-pro' ); ?></p>
								<?php
								break;
		...
		public static function headers() {
			...
			if ( $response_code !== 200 ) {
				return new \WP_Error( '_response_code_error', esc_html__( 'Ooops, The Request Code :', 'kata-plus-pro' ) . $response_code );
			}
			...
			if ( empty( $template_data ) ) {
				return new \WP_Error( '_response_code_error', esc_html__( 'Ooops, Invalid Data', 'kata-plus-pro' ) );
			}
			$counter = 1;
		...
		public static function typography() {
			$template = array(
				'1' => __( 'Open Sans', 'kata-plus-pro' ),
				'2' => __( 'Lora', 'kata-plus-pro' ),
				'3' => __( 'Poppins', 'kata-plus-pro' ),
				'4' => __( 'Nunito', 'kata-plus-pro' ),
				'5' => __( 'Sora', 'kata-plus-pro' ),
				'6' => __( 'IBM Plex Sans', 'kata-plus-pro' ),
				'7' => __( 'Roboto', 'kata-plus-pro' ),
				'8' => __( 'Oswald', 'kata-plus-pro' ),
			);
